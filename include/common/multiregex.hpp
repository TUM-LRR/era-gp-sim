...
struct MultiregexMatch {
     ...
     * \param source_ The COMPLETE source string (the relevant part will be cut out). 
 ...
 * \tparam CharT The character type used for strings and the regex. Must be integral.
 *
 * Please note that the regexes are not in any order, this is implementation-defined! Yet, it is sometimes useful to put the smaller regexes after the bigger ones.
 */
template<typename CharT>
class Multiregex {
...
public:
     ...
     * \param choice A vector of strings which will be put into the regex. Only one of them has to match. There is no order which regex in the list if preferred, this is rather implementation-defined.
    ...
    Multiregex(const String& prefix, const String& suffix, const std::vector<String>& choice, std::regex::flag_type flags = std::regex_constants::ECMAScript)
        : _choice(), _baseGroup(countBrackets(prefix) + 1), _regex(buildMultiregex(prefix, suffix, choice, flags), flags)
    {
    ...
    bool search(const String& data, MultiregexMatch<CharType>& multimatch) const
    {
        ...
        if (result)
        {
            //If we have matched, we look for the first group inside (or after, does not matter, one group will have been matched (hopefully)) which has the full length of the complete group. For this one we know that it must be a group wrapping one of our choice regexes, b/c if not, there would have been another group as big before. So we get always the first matching group.
			...
			for (size_t i = _baseGroup + 1; i < match.size(); ++i)
			{
				if (match.length(i) == targetLength)
				{
                    ...
                    assert(_choice.find(i) != _choice.end()); //(this should actually never fail, only if something in this implementation has gone wrong)
...
private:
    ...
    //Counts the number of opening brackets in the string which denote the start of a regex group.
    size_t countBrackets(const String& str) const
    {
        ...
        for (size_t i = 0; i < str.size(); ++i)
        {
            ...
            //* The bracket must not denote the start of a not-captured group/lookahead.
            if (prev != '\\' && next != '?' && str[i] == '(')
            {
                ++count;
            }
    ...
    String buildMultiregex(const String& prefix, const String& suffix, const std::vector<String>& choice, std::regex::flag_type flags)
    {
