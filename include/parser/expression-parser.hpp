 ...
 *
 * So, this class applies the Shunting Yard algorithm (oriented at http://wcipeg.com/wiki/Shunting_yard_algorithm). It knows its binary and unary operators and how to decode literals. For this version, the unary operators have one specific precedence which is always higher than the binary one. For the Shunting Yard algorithm, we do only have to go through the string once. Whilst doing that, we maintain two stacks: one stack containing operators and brackets, and the other one containing temporary numbers which cannot be evaluated yet. In the end, the operator stack should be empty and on the output stack (the one which contains the temporary numbers) is only one element. This is the value we want to return.
 * 
 * During the parsing procedure, we decide what to do depending on the token type we are currently looking at:
 * _Note:_ When we discard an operator, we apply it to the stack. For a binary operator, the stack size then decreases by one. A unary operator changes nothing at the stack size.
 *
 * * a _literal_ is parsed instantly and pushed upon the output stack of temporary numbers.
 * * an _opening bracket_ is simply pushed onto the operator stack.
 * * for an _operator_, we first of all check, if there is a closing bracket or a number preceeding it. If so, the operator is definitely binary and if not, we probably have a unary operator. So we have determined the arity of our operator. The next step is to evaluate all operators which are now finished, i.e. there cannot come any more operands/operators for them which have an equal precedence. Example: We got the string "1 * 2 + 3", then after parsing '+' we know that '*' is finished, as there can be no more operand/operator after it, which might influence the "1 * 2". As we have pushed our operators on the stack if and only if they have a lower precedence than all previous operators, we can simply pop from the stack until we get to the point that we have at most and equals precedence, but no smaller one on it any more. And we may even discard if we have already an operator of equal precedence lying on it, because if our operator is left-associative, we want to insta-evaluate the expression. If we got a right-associative operator, we want to wait until there is any operator of higher precedence or the end of the string. After we have discarded all operators with lower precedence, we push our operator onto the stack.
 * * a _closing bracket_ behaves similar to an operator which has a higher precedence than possible, but does stop at a bracket on the stack (i.e. we do view the bracket as "left-associative") which we then discard.
 *
 * After we have handled the complete string, there might be still some operators on the stack which we then all discard (by applying a more powerful operator than the one used for brackets, one which is not present on the stack at all times). Then we should be done.  
 */
template<typename T>
...
public:
	 ...
	 * \return The result of the expression if the compilation has been successful. If now, `T()` will be returned.
	 */
	T parse(const std::vector<ExpressionToken>& tokens, CompileState& externalState) const
	{
		//The parse state is only used in this class (it contains references as members).
...
private:
	...
	//This enum is for the operator stack. It is a bit different from the 'external' token enum.
	...
	//An item on the operator stack, it behaves like a token. (that is the reason for the name)
	struct IToken
	{
		ITokenType type;
		std::string data;
	...
	//A function definition for internal decoding (the regex is found in a different variable).
	...
	bool pushOperator(ParseState& state, const IToken& token) const
	{
		if (!valid(token))
		{
			//For the right error message, we got to determine the arity of our operator.
	...
	bool handleBorder(ParseState& state, const IToken& token) const
	{
		...
		//If there has been nothing on it, there are some brackets closed that have not been opened.
	...
	{
		//We define unary to be if we cannot be binary (no literal, no brackets closed).
	...
	bool parseLiteral(ParseState& state) const
	{
		...
		{
			//We need to 'tokenize' the literal again, as our previous tokenizer does only give us the plain strings. So here we got to determine which literal decoder we should use.
			T parsed;
	...
	bool getValues(ParseState& state, size_t count, std::vector<T>& output) const
	{
		...
		//We need to reverse the operands, as we push them in our vector the wrong way.
	...
	{
		//To have an empty operator stack here is completely unexpected (as this error is treated somewhere else).
		...
		case ITokenType::BRACKET:
			//We should never apply a bracket token (then, some closing brackets are missing).
	...
	{
		//To decrease the stack, we need the precedence of our operator and if we may discard operators of equal precedence.
		...
		{
			//We loop until we cannot do this any more (or there are no operators left to process).
	...
	{
		//Anything other than an operator is always valid. For operators, we look if there is an operator defined like this and with this specific arity.
		if (token.type == ITokenType::UNARY_OPERATOR)
		{
	...
	//Returns, if we should also remove tokens of same precedence from the operator stack.  
	bool popSamePrecedence(const IToken& token) const
	{
		//Only applies to binary operators as there is no associativity for anything else.
		switch (token.type)
		{
		case ITokenType::UNARY_OPERATOR:
		case ITokenType::BRACKET:
	...
	int precedence(const IToken& token) const
	{
		...
		{
		case ITokenType::UNARY_OPERATOR:
			//Unary operators always have the lowest precedence, i.e. they do not alter the stack.
			return -1;
		case ITokenType::BRACKET:
			//Brackets have a higher precedence than any other operator, i.e. they remove any other operators from the stack.
			return 0x10000;
		case ITokenType::EXPRESSION:
			//An expression has the highest precedence and is never found on the stack. It makes everything disappear.
			return 0x1ffff;
		case ITokenType::BINARY_OPERATOR:
