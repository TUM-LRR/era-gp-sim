 ...
 *
 * This expression compiler uses the Shunting Yard algorithm as described here: http://wcipeg.com/wiki/Shunting_yard_algorithm
 * Currently this thing supports compiling expressions which have brackets, binary and unary operators, while the binary operators have different precedences (the unary ones do not).
 * The compiler operates in two steps, it first splits the string into tokens (like brackets, literals, operators), then it parses it and evaluates it on the fly (so we're doing syntax and semantic analysis in the same step). It outputs a number, not an RPN or a syntax tree (if we outputted the latter, maybe with some parser clone-classes one could implement maybe full-grown x86, just speculating). How the tokenizer operates is rather simple: it just greedily tries to match the next-best regex for a token type and repeats this procedure until the whole string is tokenized. For the parser, the Shunting Yard algorithm comes in handy. Here, we evaluate the expression using two stacks (one for operators, one for output), so that we can manage different precedences. And that's it. This class takes a definition of a compiler and produces a token definitions list out of it, and also passes the parser definition to the parser itself.
 * Functions and the ternary operators are currently not supported, though possible, because that would lead to some really complicated code with huge switch-cases and that is not what we want (of course, one could implement each token type as a separate class/function struct which is then called, that might make it doable). Also, there is no real practical need for the ternary (we're writing assembler after all, all that could vary are our constants), and functions are probably not needed, though they provide some useful operations sometimes. Probably the best idea to support the two and also stuff like [eax+ebx*8+1234] is to write a full-grown LR parser or something similar, then one may implement this all and must only supply a table with parser data.
 */
template<typename T>
...
public:
	...
	/**
	 * \brief Creates a new expression compiler with the given tokenizer and parser.
	...
	{
		//Pretty simple: Just pass the string and the tokens in the tokenizer and parser respectively.
...
private:
	...
	std::vector<ExpressionTokenDefinition> getTokenDefinitions(const ExpressionCompilerDefinition<T>& definition)
	{
		...
		//But still, we need to escape the tokens, as they are probably also using characters which are occupied by regex functions.
		...
		//For the literal decoder tokens, it is simpler, as they are already regexes.
