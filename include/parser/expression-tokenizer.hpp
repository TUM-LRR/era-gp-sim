 ...
 *
 * This system uses a single regex which contains code to match all possible tokens (this way, we may emulate parsing using a finite automaton). If we match, we shorten the string and match again and again until nothing is left. If we do not match any token, the string is seen as invalid.
 */
...
private:
	...
	std::vector<std::string> buildRegexVector(const std::vector<ExpressionTokenDefinition>& definitions)
	{
		...
		//We copy the regexes from our definitions and also include a whitespace finder.
		output.push_back("\\s+");
