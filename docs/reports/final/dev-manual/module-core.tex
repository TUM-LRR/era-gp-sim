...
\texttt{std::vector<uint8\_t>} in konstanter Komplexität einfach zu
realisieren, während der Zugriff auf irgendwelche Blöcke in einem
\texttt{std::vector<bool>} äußerst umständlich oder ineffizient wäre. Der
Datentyp, welcher im folgenden \texttt{MemoryValue}, da dieser Werte(engl.
Value) aus dem Speicher(engl. Memory) enthält, genannt wird, ist daher über
einem \texttt{std::vector<uint8\_t>} aufgebaut.
...
\begin{figure}[H]
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
\caption{Darstellung einer beispielhaften Implementierung eines Registersatzes
mit einer Map abbildend Registerbezeichner auf Registeridentifikator links, den
Elterregistern in der Mitte und Listen an zu benachrichtigenden Registern in
der GUI}
