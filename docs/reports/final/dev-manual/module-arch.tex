...
eines Syntaxbaums. Neben der syntaktischen Überprüfung des Parsers, validiert
die Architektur die semantische Korrektheit einer Instruktion, und stellt bei
...
Die Architektur soll in der Lage sein, beliebig komplexe Assembler Instruktionen
in einem einheitlichen Format modellieren zu können. Deshalb haben wir uns für
die Verwendung eines Syntaxbaums zur Darstellung einer Instruktion
entschieden. \\
Eine beispielhafte Ausprägung eines Syntaxbaums, der folgende x86 Instruktion
modelliert, ist in \autoref{fig:arch-syntax-tree-example} gegeben.
\begin{x86}
add eax, [4*ebx+8]
\end{x86}
...
\begin{figure}[H]
	\begin{center}
	\begin{tikzpicture}
	\Tree [.{\texttt{add}} \texttt{eax} [.{Memory Access} [.{Addition} [.{Multiplication} 4 \texttt{ebx} ] 8 ] ] ]
	\end{tikzpicture}
	\end{center}

	\caption{Beispielhafter Syntaxbaum}
	\label{fig:arch-syntax-tree-example}
\end{figure}

Aufbauend auf diesem Konzept, haben wir verschiedene Knotentypen
herausgearbeitet, die im Folgenden erläutert werden.

Der Syntaxbaum wird durch Vererbung implementiert und in \textbf{common} und
\textbf{riscv} (oder jede beliebige andere Architektur) gegliedert. Eine
...
\begin{itemize}
  ...
  \item \textbf{ImmediateNode} repräsentiert einen konstanten Wert (Immediate),
  ...
  Konkret wird er für die Implementierung der Crash Instruktion verwendet.
	...
	Instruktion. Um die Architekturen-spezifische Assemblierung der
	Register zu unterstützen, ist diese Klasse abstrakt implementiert.
...
x86 wurde bereits in \autoref{fig:arch-syntax-tree-example} aufgeführt.
...
\begin{itemize}
	...
	Instruktionen wird über den Syntaxbaum vorgenommen.
...
Mit der \textit{Node Factory} wird der bereits beschriebene Syntaxbaum erzeugt.
Um nach außen ein Architektur-unabhängiges Interface zu bieten, basiert die
Node Factory dem \textit{Abstract Factory Pattern}
\footnote{\url{https://de.wikipedia.org/wiki/Fabrikmethode}} und wird als
Diagramm in \autoref{fig:arch-node-factory} dargestellt.
...
\begin{figure}[H]
	\begin{center}
		...
		\draw[inheritance-arrow] (instr-riscv) edge node [yshift=3mm] {erben} (instr-abstr);
		...
		\path (arithmetic-abstr.east |- arithmetic-abstr.south)+(+0.7,-1.5) node (a2) {};
		\path[rounded corners, draw=black!50, dashed] (a1) rectangle (a2);
		\end{pgfonlayer}
...
Zunächst etwas zur Implementation der Instruktionen. Für RISC-V definieren
wir wiederum eine eigene abstrakte Oberklasse \texttt{riscv::InstructionNode},
von der alle weiteren Instruktionsknoten abgeleitet sind. Dies dient der
Vermeidung von Redundanz, da Methoden wie \texttt{assemble()} und
...
Ein weiteres Konzept der RISC-V Implementation von Instruktionen basiert auf der
Unterstützung unterschiedlicher Wortgrößen. Derzeit bietet RISC-V ausgiebige
Unterstützung für 32 und 64 Bit, in Zukunft soll 128 Bit folgen. Um zu
verhindern, dass Instruktionen für jede Wortgröße neu geschrieben werden müssen,
nutzen wir C++ Templates, um die Wortgröße einer Instruktion zu spezifizieren.
So wird in der Node Factory von RISC-V eine Fallunterscheidung nach der
verwendeten Wortgröße gemacht, und dann der entsprechende Zahlentyp als Template
Parameter gesetzt (für 32 Bit z.B. \texttt{std::uint32\_t} und für 64 Bit
