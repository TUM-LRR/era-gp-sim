...
Zur abstrahierten Darstellung von ausführbaren Instruktionen wird ein Syntaxbaum verwendet.
Jede Instruktion, die der Parser einliest, resultiert in einen Syntaxbaum; ein Programm ist also
eine Liste dieser Bäume. Dabei bildet der Instruktionsknoten die Wurzel, seine Operanden werden als Kinder an den Baum gehängt.
Der Syntaxbaum ist durch Vererbung implementiert und in \emph{common} und
\emph{riscv} (bzw. den konkreten Teil jeder Architektur) gegliedert. Eine
...
Architekturen stellen aber häufig die Möglichkeit bereit, einen
Speicherzugriff im Zuge einer anderen Instruktion durchzuführen. Ein Beispiel aus
x86:
