...
\begin{center}
	...
	\uncover<2->{\node (op) [draw = black] {\textbf{Operation}};}
    
    \uncover<3->{\node (dir) [draw = black, right = of op] {\textbf{Direktive}};
    \node (inst) [draw = black, above = of dir] {\textbf{Instruktion}};
    \node (mac_inst) [draw = black, below = of dir] {\textbf{Makro-Instr.}};
    
    \draw [inher] (dir) -- (op);
    \draw [myline] (op.east) ++(1cm,0) |- (inst);
    \draw [myline] (op.east) ++(1cm,0) |- (mac_inst);}
    
    \uncover<4->{\node (sec) [draw = black, right = of dir] {\textbf{Sektion}};}
    \uncover<6->{\node (mac) [draw = black, above = of sec] {\textbf{Makro}};}
    \uncover<5->{\node (const) [draw = black, above = of mac] {\textbf{Konstante}};}
    \uncover<7->{\node (res) [draw = black, below = of sec, align = center] {\textbf{Speicher}\\\textbf{Reservierung}};
    \node (def) [draw = black, below = of res, align = center] {\textbf{Speicher}\\\textbf{Definition}};}
