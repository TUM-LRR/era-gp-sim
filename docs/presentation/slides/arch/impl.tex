...
\begin{slide}{Beispiel}
  \LARGE\texttt{add eax, [6*ebx+ecx+9]}
\end{slide}
...
\begin{slide}{Beispiel}
  ...
  \begin{tikzpicture}[thick]
    ...
    %%%%%%%%
    \onslide<1>{
      \node (code) at (8, -4.5) {\LARGE\texttt{add eax, [6*ebx+ecx+9]}};
    }
    \onslide<2>{
      \node (code) at (8, -4.5) {%
        \LARGE\texttt{\textcolor{Purple}{add} eax, [6*ebx+ecx+9]}
      };
    }
    \onslide<3>{
      \node (code) at (8, -4.5) {%
        \LARGE\texttt{add \textcolor{Green}{eax}, %
        \textcolor{ProcessBlue}{[}6*ebx+ecx+9\textcolor{ProcessBlue}{]}}
      };
    }
    \onslide<4>{
      \node (code) at (8, -4.5) {%
        \LARGE\texttt{add eax, [6*ebx+ecx\textcolor{orange}{+}9]}
      };
    }
    \onslide<5>{
      \node (code) at (8, -4.5) {%
        \LARGE\texttt{add eax,%
        [6*ebx\textcolor{orange}{+}ecx+\textcolor{Red}{9}]}
      };
    }
    \onslide<6>{
      \node (code) at (8, -4.5) {%
        \LARGE\texttt{add eax,%
        [6\textcolor{orange}{*}ebx+\textcolor{Green}{ecx}+9]}
      };
    }
    \onslide<7>{
      \node (code) at (8, -4.5) {%
        \LARGE\texttt{add eax,%
        [\textcolor{Red}{6}*\textcolor{Green}{ebx}+ecx+9]}};
    }
    ...
    % Level 5 (6, ebx)
    \onslide<7->{
      \path (-2, -7.5) coordinate [Red, node] (6) node {\texttt{6}};
