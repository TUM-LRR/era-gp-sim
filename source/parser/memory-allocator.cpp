...
RelativeMemoryPosition::RelativeMemoryPosition(const std::string& section, std::size_t offset)
...
MemorySectionDefinition::MemorySectionDefinition(const std::string& name, std::size_t sectionAlignment, std::size_t dataAlignment)
    ...
    MemorySectionDefinition::MemorySectionDefinition(const std::string& name, std::size_t alignment)
    ...
    void MemoryAllocator::MemorySection::clear()
        ...
        std::size_t MemoryAllocator::MemorySection::absoluteAddress(std::size_t relative) const
        {
            return relative + _currentPosition;
        }

        MemoryAllocator::MemorySection::MemorySection(const MemorySectionDefinition& definition)
            : _definition(definition), _currentSize(0), _currentPosition(0)
        {
            
        }

        MemoryAllocator::MemoryAllocator(const std::vector<MemorySectionDefinition>& sectionDefinitions)
    {
    ...
    std::size_t MemoryAllocator::absolutePosition(const RelativeMemoryPosition& relative) const
    {
