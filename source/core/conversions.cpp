...
MemoryValue permute(const MemoryValue& memoryValue, const std::size_t byteSize, const std::vector<std::size_t> permutation) {
  assert(byteSize > 0);
  assert(memoryValue.getSize() % byteSize == 0);//The value has to be made out of whole bytes
  const std::size_t byteAmount{ memoryValue.getSize() / byteSize };
  assert(permutation.size() >= byteAmount);//We need at least byteAmount values to permute
  ...
  for (std::size_t i = 0; i < byteAmount; ++i) {
    for (std::size_t j = 0; j < byteSize - 1; j += 8) {
      if (j + 8 < byteSize) {
        permuted.push_back(memoryValue.getByteAt(permutation[i] * byteSize + j));
      }
      else {
        permuted.push_back(memoryValue.getByteAt(permutation[i] * byteSize + j) & mask);
      }
    }
  }
  ...
  for (std::size_t i = 0; i < byteAmount; ++i) {
    for (std::size_t j = 0; j < sizeOfByte; ++j) {
      ...
      if (bitsInBuffer >= 8) {
        ...
        //the part of the current byte that didn'tfit into the Buffer become the new Buffer
        bufferByte = permuted[i*sizeOfByte + j] >> (16 - bitsInBuffer);//(8 - (bIB % 8))
        bitsInBuffer -= 8;
      }
    }
  }
