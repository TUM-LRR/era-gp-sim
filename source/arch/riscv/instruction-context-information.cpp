...
bool InstructionContextInformation::isContextInformationAvailable(const std::string &mnemonic) const {
    std::string key = Utility::toLower(mnemonic);
    return _table.find(key) != _table.end();
}

bool InstructionContextInformation::isContextInformationAvailable(const InstructionInformation &instructionInfo) const {
...
void InstructionContextInformation::_fill(const Architecture &architecture) {
  ...
  _add("slt",
       DocumentationBuilder()
           ...
           .detailDescription(RISCV_TR("Performs a signed comparison of rs1 and rs2. If "
  ...
  _add("sltu",
       DocumentationBuilder()
           ...
           .operandDescription("rs1", RISCV_TR("First source register"))
           .operandDescription("rs2", RISCV_TR("Second source register"))
           .detailDescription(RISCV_TR("Performs an unsigned comparison of rs1 and rs2. "
  ...
  _add("slti",
       DocumentationBuilder()
           ...
           .detailDescription(RISCV_TR("Performs a signed comparison of rs and imm. If "
  ...
  _add("sltiu",
       DocumentationBuilder()
           ...
           .operandDescription("rs", RISCV_TR("The source register"))
           .operandDescription("imm", RISCV_TR("A signed 12 bit immediate"))
           .detailDescription(RISCV_TR("Performs an unsigned comparison of rs and imm. "
  ...
  _add("lui",
       DocumentationBuilder()
           ...
           .detailDescription(RISCV_TR("Places the sign extended immediate into bit 31 "
  ...
  _add("jal",
       DocumentationBuilder()
           ...
           .operandDescription(
               "rd",
               RISCV_TR("Register where the address of the next instruction is stored"))
  ...
  _branchInstruction("blt", RISCV_TR("less than"), "<", RISCV_TR(" (signed compare)"));
  _branchInstruction("bltu", RISCV_TR("less than"), RISCV_TR(" (unsigned compare)"));
  _branchInstruction("bge", RISCV_TR("greater or equal to"), ">=", RISCV_TR(" (signed compare)"));
  ...
  _add("simusleep",
       DocumentationBuilder()
           ...
           .shortSyntax({"ms"})
           .operandDescription("ms", RISCV_TR("The time to sleep in milliseconds"))
           .detailDescription(RISCV_TR("Holds the execution of the next instruction at "
  ...
  _add("simucrash",
       DocumentationBuilder()
           ...
           .shortSyntax({"msg"})
           .operandDescription(
               "msg", RISCV_TR("Message to be shown when this instruction is executed"))
           .detailDescription(RISCV_TR("Terminates the running program showing msg"))
           .build());
  ...
  if (_is64BitArchitecture) {
    _arithmeticInstructionIW(
        "add", "+",
        RISCV_TR("Calculates the sum of the lower 32bit of %1 and the lower 32bit of %2 "
        "and stores the "
    ...
    _arithmeticInstructionIW(
        "sll", "<<",
        RISCV_TR("(Logical) shifts the content of the lower 32bit of %1 by %2 bits to "
        "the left, inserting 0s in bit 0.<br>Note that only "
    ...
    _arithmeticInstructionIW(
        "sra", "<<",
        RISCV_TR("(Arithmetical) shifts the content of the lower 32bit of %1 by %2 bits "
        "to the right, inserting the sign bit.<br>Note that "
        "only the lower 5bit of %2 determine the shift "
        "amount. The result is sign extended to 64bit"),
        RISCV_TR("A 5bit unsigned immediate"));
  ...
  if (architecture.getInstructions().hasInstruction("mul")) {
    ...
    _mInstruction("mulh", "*", RISCV_TR("signed x signed multiplication"),
                  RISCV_TR("register used for the multiplicand"),
                  RISCV_TR("register used for the multiplier"), RISCV_TR("upper"));
    ...
    _mInstruction("div", "/", RISCV_TR("signed-signed division"),
                  RISCV_TR("register used as dividend"), RISCV_TR("register used as divisor"));
    ...
    if (_is64BitArchitecture) {
      _mInstruction("mulw", "*", RISCV_TR("signed 32bit x signed 32bit multiplication"),
                    RISCV_TR("register used for the multiplicand"),
                    RISCV_TR("register used for the multiplier"), RISCV_TR("lower"));
      _mInstruction("divw", "/", RISCV_TR("signed 32bit -signed 32bit division"),
                    RISCV_TR("register used as dividend"), RISCV_TR("register used as divisor"));
      _mInstruction("divuw", "/", RISCV_TR("unsigned 32bit -unsigned 32bit division"),
                    RISCV_TR("register used as dividend"), RISCV_TR("register used as divisor"));
      _mInstruction("remw", "%", RISCV_TR("signed 32bit remainder operation"),
                    RISCV_TR("register used as dividend"), RISCV_TR("register used as divisor"));
      _mInstruction("remuw", "%", RISCV_TR("unsigned 32bit remainder operation"),
                    RISCV_TR("register used as dividend"), RISCV_TR("register used as divisor"));
    }
  }
}
...
void InstructionContextInformation::_arithmeticInstruction(
  ...
  builder
      .operandDescription(rd, RISCV_TR("The destination register"))
      .operandDescription(
          op1, isImmediate ? RISCV_TR("The operand register")
...
void InstructionContextInformation::_loadInstruction(
  ...
  builder.operandDescription("rd",
                             RISCV_TR("The register, the loaded value is stored to"));
  ...
  auto detailTranslate = std::make_shared<Translateable>(RISCV_TR("Loads one %1(%2 bit) into the register rd%3%4"));
  detailTranslate->addOperand(sizeDesc);
  detailTranslate->addOperand(std::to_string(size));
  if (mnemonic.back() != 'u' && mnemonic.back() != 'U') {
    detailTranslate->addOperand(RISCV_TR(" performing a sign extension on the whole register if needed"));
  }else{
      detailTranslate->addOperand("");
  }
  detailTranslate->addOperand(RISCV_TR(". The memory address is determined by adding base and offset."));
...
void InstructionContextInformation::_branchInstruction(
  ...
  auto detail = std::make_shared<Translateable>(RISCV_TR("Compares rs1 and rs2. If rs1 is %1 rs2%2, the program jumps to the label or instruction address determined by adding the program counter to 2x offset. If not no jump is made."));
...
void InstructionContextInformation::_mInstruction(
  ...
  builder
      .operandDescription("rd",
                          RISCV_TR("Destination register where the result is stored"))
      .operandDescription("rs1", operand1Desc)
      .operandDescription("rs2", operand2Desc);
  auto detail = std::make_shared<Translateable>(RISCV_TR("Performs a %1 with rs1 and rs2 and stores the %2%3 of the result in the destination register rd"));
