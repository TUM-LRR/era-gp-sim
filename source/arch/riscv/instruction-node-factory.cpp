...
template<typename UnsignedWordSize, typename SignedWordSize>
void initialize64BitWordInstructions(InstructionNodeFactory::InstructionMap& _instructionMap) {
    _instructionMap.emplace("mulw", [](InstructionInformation& info) {
      return std::make_unique<MultiplicationInstruction<UnsignedWordSize>>(
          info, MultiplicationInstruction<UnsignedWordSize>::LOW,
          MultiplicationInstruction<UnsignedWordSize>::SIGNED);
    });
    _instructionMap.emplace("divw", [](InstructionInformation& info) {
        return std::make_unique<DivisionInstruction<UnsignedWordSize, SignedWordSize>>(info, true);
    });
    _instructionMap.emplace("divuw", [](InstructionInformation& info) {
        return std::make_unique<DivisionInstruction<UnsignedWordSize, SignedWordSize>>(info, false);
    });
    _instructionMap.emplace("remw", [](InstructionInformation& info) {
        return std::make_unique<RemainderInstruction<UnsignedWordSize, SignedWordSize>>(info, true);
    });
...
void InstructionNodeFactory::initializeInstructionMap(
  ...
  if (wordSize == InstructionNodeFactory::RV32) {
    ...
    initialize64BitWordInstructions<InstructionNodeFactory::RV32_integral_t, InstructionNodeFactory::RV32_signed_integral_t>(_instructionMap);
  } else {
