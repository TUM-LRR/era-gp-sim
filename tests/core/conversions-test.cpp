...
static_assert(MAXIMUM_BYTE_SIZE <= MAXIMUM_BITS, "Maximum bit count per byte exceeds total maximum bit count");
 ...
 */
std::pair<uint64_t, uint64_t> getIntegers(const std::vector<uint8_t>& data,
...
std::pair<std::pair<uint64_t, uint64_t>, MemoryValue> generateMemoryValue(
      std::size_t bytes,
      std::size_t bitsPerByte,
      RandomEngine& engine,
  ...
  if (bitsPerByte % 8 != 0) {
    for (std::size_t item = internalItemsPerByte - 1;
         item < internalItemCount;
  ...
  return std::pair<std::pair<uint64_t, uint64_t>, MemoryValue>(
    integers,
    MemoryValue(std::move(internalData), bitsPerByte)
  );
}
...
// Standard conversions - test the result of convert(...) against the expected
// result value
...
TEST(TestUnsignedConversions, numberToMemoryValue) {
  ...
  for (std::size_t iteration = 0; iteration < REPETITIONS; ++iteration) {
    for (std::size_t byteSize = 1; byteSize < MAXIMUM_BYTE_SIZE; ++byteSize) {
      ...
      MemoryValue valueBE = convert(random, byteSize, Endianness::BIG);
      MemoryValue valueLE = convert(random, byteSize, Endianness::LITTLE);
...
// "There And Back Again" - verify that the result of convert(convert(...)) is
// equal to the input
  ...
  for (std::size_t iteration = 0; iteration < REPETITIONS; ++iteration) {
    for (std::size_t byteSize = 1; byteSize < MAXIMUM_BYTE_SIZE; ++byteSize) {
      for (std::size_t bytes = 1; bytes * byteSize < MAXIMUM_BITS; ++bytes) {
        ...
        MemoryValue resultBE = convert(valueBE, byteSize, Endianness::BIG,
                                       bytes);
        MemoryValue resultLE = convert(valueLE, byteSize, Endianness::LITTLE,
                                       bytes);
    ...
    for (auto representation : {SignedRepresentation::ONES_COMPLEMENT, 
      ...
      for (std::size_t byteSize = 1; byteSize < MAXIMUM_BYTE_SIZE; ++byteSize) {
        for (std::size_t bytes = 1; bytes * byteSize < MAXIMUM_BITS; ++bytes) {
          ...
          // Convert as big and little endian
          int64_t valueBE = convert<int64_t>(random, Endianness::BIG,
                                             representation);
          int64_t valueLE = convert<int64_t>(random, Endianness::LITTLE,
                                             representation);

          // Verify conversions
          ASSERT_EQ(toSigned(bigEndian, representation, bytes * byteSize), valueBE);
          ASSERT_EQ(toSigned(littleEndian, representation, bytes * byteSize), valueLE);
        }
      }
    }
  }
    ...
    for (auto representation : {SignedRepresentation::ONES_COMPLEMENT, 
      ...
      for (std::size_t byteSize = 1; byteSize < MAXIMUM_BYTE_SIZE; ++byteSize) {
        for (std::size_t width = 1; width < MAXIMUM_BITS; ++width) {
          ...
          // Convert to signed
          int64_t random = toSigned(randomUnsigned, representation,
                                    bytesUsed * byteSize);
          ...
          int64_t little = toSigned(compareLE, representation, valueLE.getSize());
    ...
    for (auto representation : {SignedRepresentation::ONES_COMPLEMENT, 
      ...
      for (std::size_t byteSize = 1; byteSize < MAXIMUM_BYTE_SIZE; ++byteSize) {
        for (std::size_t width = 1; width < MAXIMUM_BITS; ++width) {
          ...
          int64_t resultBE = convert<int64_t>(valueBE, Endianness::BIG,
                                              representation);
          int64_t resultLE = convert<int64_t>(valueLE, Endianness::LITTLE,
                                              representation);
